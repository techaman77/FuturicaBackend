...
const login = async (req, res) => {
  ...
  try {
    ...
    if (user.role === "admin") {
      ...
      //     "Otp required for login. Please contact admin.",
      //     403
      //   );
    }
    ...
    if (today.getDay() === "Sunday") {
      throw new CustomError(
        `You aren't allowed to work on Sundays. Enjoy!`,
        500
      );
    }
...
const verificationOtp = async (req, res) => {
  ...
  try {
    ...
    user.workLogs = user.workLogs?.filter((log) => {
...
const logout = async (req, res) => {
  ...
  try {
    ...
    let lastRecord = filterLogs[filterLogs.length - 1];
    let lastSecondRecord = filterLogs[filterLogs.length - 2];
